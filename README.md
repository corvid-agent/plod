...
Read-only snapshot as of round `67143547` (2026-09-09T17:47:15Z UTC): `docs/snapshot.json`. Refresh with `python3 scripts/refresh_snapshot.py` (no key). The CRT prefers live algod/indexer and falls back to that file. CRT graphs paint appending TestNet samples from `docs/history.json` via in-page sql.js (filtered to app 770734249 / upkeep 110).
