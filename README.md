...
Read-only snapshot as of round `67402244` (2026-09-17T19:28:01Z UTC): `docs/snapshot.json`. Refresh with `python3 scripts/refresh_snapshot.py` (no key). The CRT prefers live algod/indexer and falls back to that file. CRT graphs paint appending TestNet samples from `docs/history.json` via in-page sql.js (filtered to app 770734249 / upkeep 110).
...
- Second tick landed; as of snapshot round **67402244** (2026-09-17T19:28:01Z): times_executed / `calls` **2**, last tick round **67278250**, next due **67502248**, escrow **492000** µALGO, status **ON TIME**
